...
const RenderCommonInputField = ({
  ...
  return <>
    <Form.Control onKeyPress={onKeyPress}
                  ...
                  type={type}
                  readOnly={readOnly}
                  className={`_custom-input form-control ${isHideBorder ? 'border-0' +
                    ' transparent px-0' : ''} 
                  ${touched && error ? 'has-error mb-0' : ''} ${readOnly ? 'bgDisable' : ''} `}
                  id={input.name}
...
export default RenderCommonInputField;
